...
for jsonFile in jsonAnnots:
  with open(jsonFile) as f:
    ...
    annotationData = json.loads(json.loads(jsonData[0]['consolidatedAnnotation'][
      ...
      'content'])['annotations'])
    print('what are annotations?')
    print(annotationData)
    print('first element:', annotationData[0])
    ...
    if len(cocoOutput['categories']) == 0:
      ...
      print('loaded the image?', img.shape)
...
#     # cv2.waitKey(0)
    ...
    for instance in annotationData:
      polygonPts = np.multiply(np.asarray(instance['data']).flatten(), img.shape[1] / 1000)
      blankImg = Image.new("L", tuple(reversed(img.shape[0:2])), 0)
      ImageDraw.Draw(blankImg).polygon([int(el) for el in polygonPts], outline=1, fill=1)
...
#           annotation["segmentation"].append(segmentation)
      annotation['segmentation'].append(list(polygonPts))
